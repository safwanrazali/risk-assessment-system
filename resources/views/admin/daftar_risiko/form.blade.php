...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ $daftar->exists ? route('admin.daftar_risiko.update', $daftar) : route('admin.daftar_risiko.store') }}">
            @csrf
            @if($daftar->exists)
                @method('PUT')
            @endif
            ...
            <div class="d-flex gap-2 mt-3">
                ...
                <a href="{{ route('admin.daftar_risiko.index') }}" class="btn btn-secondary">Kembali</a>
            </div>
        </form>
    </div>
</div>
