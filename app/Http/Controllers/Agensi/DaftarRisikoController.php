...
class DaftarRisikoController extends Controller
{
    ...
    public function index(): View
    {
        ...
        return view('agensi.daftar_risiko.index', compact('items'));
    }
    ...
    public function create(): View
    {
        ...
        return view('agensi.daftar_risiko.form', compact(
    ...
    public function store(DaftarRisikoRequest $request): RedirectResponse
    {
        ...
        return redirect()->route('agensi.daftar_risiko.index')->with('success', 'Risiko berjaya didaftarkan.');
    }
    ...
    public function update(DaftarRisikoRequest $request, DaftarRisiko $daftar_risiko): RedirectResponse
    {
        ...
        return redirect()->route('agensi.daftar_risiko.index')->with('success', 'Risiko berjaya dikemas kini.');
    }

    public function destroy(DaftarRisiko $daftar_risiko): RedirectResponse
    {
        $this->service->delete($daftar_risiko);
        return redirect()->route('agensi.daftar_risiko.index')->with('success', 'Risiko berjaya dipadam.');
    }
    ...
    public function laporan(): View
    {
        ...
        // Get all items for this agency (paginated)
        $items = $this->service->query()
            ->where('agensi_id', $agensiId)
            ->latest()
            ->paginate(10);
        
        // Highest risk in this agency (paginated with through to preserve paginator)
        $highestRiskByAgency = DaftarRisiko::where('agensi_id', $agensiId)
            ->select('agensi_id')
        ...
        // Asset type with highest total risk (for this agency, paginated with through to preserve paginator)
        $highestRiskByAssetType = DaftarRisiko::where('daftar_risikos.agensi_id', $agensiId)
            ->join('asets', 'daftar_risikos.aset_id', '=', 'asets.id')
        ...
        return view('agensi.daftar_risiko.laporan', compact('items', 'highestRiskByAgency', 'highestRiskByAssetType'));
    }
    ...
    public function downloadPdf()
    {
        ...
        // Asset type with highest total risk (for this agency)
        $highestRiskByAssetType = DaftarRisiko::where('daftar_risikos.agensi_id', $agensiId)
            ->join('asets', 'daftar_risikos.aset_id', '=', 'asets.id')
            ->join('jenis_asets', 'asets.jenis_aset_id', '=', 'jenis_asets.id')
            ->select('jenis_asets.id', 'jenis_asets.nama')
    ...
    public function downloadAgencyCsv()
    {
        $agensiId = Auth::user()->agensi_id;
        $export = new LaporanRisikoExport();
        $csv = $export->exportAgencyCsv($agensiId);
    ...
    public function downloadAssetCsv()
    {
        $agensiId = Auth::user()->agensi_id;
        $export = new LaporanRisikoExport();
        $csv = $export->exportAssetCsv($agensiId);
    ...
    public function downloadDetailCsv()
    {
        $agensiId = Auth::user()->agensi_id;
        $export = new LaporanRisikoExport();
        $csv = $export->exportDetailCsv($agensiId);
